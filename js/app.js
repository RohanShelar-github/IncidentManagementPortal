window.onerror = function(msg, src, line, col, err) {
  ...
  if (!d) { d = document.createElement('div'); d.id='_errbox';
...
function addAudit(icon, action, detail) {
  auditLog.unshift({ icon: icon, action: action, detail: detail,
...
function renderFeed(incId) {
  ...
  var typeColor = { create:'var(--accent)', status:'var(--warning)', comment:'var(--text)',
...
(function seedNotifs() {
  ...
  items.forEach(function(n, i) {
    notifications.push({ id: now - i*1000, type:n.type, text:n.text,
      incId:n.incId, time:n.time, unread:i < 3 });
  });
})();

function addNotification(type, text, incId) {
  var n = { id: Date.now(), type: type || 'info', text: text || '',
...
function runCmdSearch(q) {
  ...
  incidents.filter(function(i) {
    ...
    results.push({ label: i.id + ' \u2014 ' + i.title, sub: i.customer + ' \u00b7 ' + i.severity,
  ...
  pages.filter(function(p) { return !q || p.label.toLowerCase().includes(q); })
    .forEach(function(p) {
      results.push({ label: p.label, sub: 'Navigate to page', icon: p.icon,
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // ── Load incidents from backend if enabled ─────────────────
  loadIncidentsFromBackend(function() {
    // After incidents are loaded, populate dropdowns
    populateCustomerDropdowns();
    populateAreaDropdowns();
    populateAssigneeFilter();
    updateTagFilter();
  ...
  if (hash && validPages.indexOf(hash) >= 0) {
    setTimeout(function() { navigate(hash); }, 100);
  }
