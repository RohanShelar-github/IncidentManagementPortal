...
test('close incident modal includes the existing Resolved By choices', () => {
  ...
  assert.match(html, /id="dtm_resolved_by" required aria-required="true"/);
...
test('close incident flow prefills and saves Resolved By', () => {
  ...
  assert.match(frontend, /&& inc\.rca && inc\.resolution && inc\.resolvedBy && inc\.downtimeEnd/);
  assert.match(frontend, /Please select who resolved the incident before closing it/);
});
